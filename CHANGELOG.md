...
### 變更
- links 檢查從三個檔擴大到所有被 git 追蹤的 .md（冷啟動視角）：一般檔斷連結阻斷，archive 內只警告（搬進退役區多一層後相對路徑常沒跟著調）。跳過 `_templates/`、程式碼區塊、`[[wiki-link]](括號註解)` 樣式、`[tool.anvil].links_exclude` 的 glob、含 `<!-- anvil:links-ignore -->` 的檔。來源是第一次真實掛接的全庫掃描抓到一個三檔範圍看不到的真斷連結，以及兩種已知的假警報樣式。

### 新增
- `ctx.milestone_routes(base)`：發現多路線專案的路線與各自最大里程碑編號，不假設巢狀深度。
- `docs/extending.md` 加「寫多路線檢查時的兩個坑」：巢狀深度不寫死、版本號按路線分組比對。來源是第一次真實掛接時抓到的兩個 bug，屬於任何巢狀深度不一致的專案都會踩的通用坑。
